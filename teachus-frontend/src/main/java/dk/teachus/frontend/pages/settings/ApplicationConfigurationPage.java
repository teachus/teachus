...
public class ApplicationConfigurationPage extends AuthenticatedBasePage {
	...
	public ApplicationConfigurationPage() {
		...
		form.addElement(new StringTextFieldElement(TeachUsSession.get().getString("ApplicationConfigurationPage.serverUrl"), new ApplicationConfigurationModel<String>(ApplicationConfiguration.SERVER_URL), true, 40)); //$NON-NLS-1$
		
		form.addElement(new StringTextFieldElement(TeachUsSession.get().getString("ApplicationConfigurationPage.googleAnalyticsWebPropertyId"), new ApplicationConfigurationModel<String>(ApplicationConfiguration.GOOGLE_ANALYTICS_WEB_PROPERTY_ID), false, 15)); //$NON-NLS-1$
		
		IModel<TimeZone> timeZoneModel = new ApplicationConfigurationModel<TimeZone>(ApplicationConfiguration.DEFAULT_TIMEZONE) {
			private static final long serialVersionUID = 1L;
			
			@Override
			protected TimeZone convertConfigurationValue(String configurationValue) {
				return Strings.isEmpty(configurationValue) == false ? TimeZone.getTimeZone(configurationValue) : null;
			}
			
			@Override
			protected String convertModelValue(TimeZone modelValue) {
